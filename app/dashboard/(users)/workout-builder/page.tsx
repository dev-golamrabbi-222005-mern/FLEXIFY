...
const bodyParts = [
  ...
  "calves", "biceps", "triceps", "forearms", "abdominals", "traps", "adductors", "abductors"
];
const levels = ["beginner", "intermediate", "expert"];
const equipments = ["body only", "dumbbell", "barbell", "kettlebells", "cable", "machine", "bands"];
...
export default function WorkoutBuilder() {
  ...
  const stats = useMemo(() => {
    ...
    bodyParts.forEach(part => {
      if (part === "back") {
        counts[part] = exercisesData.filter(ex => BACK_SUB_MUSCLES.some(m => ex.primaryMuscles.includes(m))).length;
      } else {
        counts[part] = exercisesData.filter(ex => ex.primaryMuscles.includes(part)).length;
      }
    });
  ...
  }, [search, selectedLevel, selectedEquipment, selectedBodyPart, exercisesData]);

  const isFilterActive = search || selectedLevel || selectedEquipment || selectedBodyPart;

  const toggleExercise = (ex: any) => {
    setSelectedExercises((prev) =>
      prev.find((i) => i.id === ex.id) ? prev.filter((i) => i.id !== ex.id) : [...prev, ex]
    );
  };
    ...
    <div className="w-full max-w-full overflow-x-hidden px-4 py-8 pb-48 bg-[var(--bg-primary)] min-h-screen">
      ...
      <div className="sticky top-0 z-40 bg-[var(--bg-primary)]/80 backdrop-blur-md pb-6 space-y-6">
        <div className="relative">
          ...
          {isFilterActive && (
            <button 
              onClick={() => {setSearch(""); setSelectedLevel(""); setSelectedEquipment(""); setSelectedBodyPart("");}} 
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar items-center py-1">
            <span className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-wider">Level:</span>
            {levels.map(l => (
              <button 
                key={l} 
                onClick={() => setSelectedLevel(selectedLevel === l ? "" : l)} 
                className={`whitespace-nowrap px-5 py-2 rounded-xl text-[10px] font-bold uppercase transition-all border ${selectedLevel === l ? 'bg-[var(--primary)] text-white border-[var(--primary)] shadow-md shadow-[var(--primary)]/20' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] border-[var(--border-color)] hover:border-[var(--primary)]'}`}
              >
                {l}
              </button>
            ))}
          ...
          {/* Muscle Filter */}
          <div className="flex gap-3 overflow-x-auto no-scrollbar items-center py-1">
            <span className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-wider">Muscle:</span>
            {bodyParts.map(p => (
              <button 
                key={p} 
                onClick={() => setSelectedBodyPart(selectedBodyPart === p ? "" : p)} 
                className={`whitespace-nowrap px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all border flex items-center gap-2 ${selectedBodyPart === p ? 'bg-[var(--primary)] text-white border-[var(--primary)]' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] border-[var(--border-color)] hover:border-[var(--primary)]'}`}
              >
                {p} <span className={`text-[9px] px-1.5 py-0.5 rounded-md ${selectedBodyPart === p ? 'bg-white/20' : 'bg-[var(--bg-tertiary)]'}`}>{stats[p] || 0}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {isFilterActive ? (
          <div>
            <div className="flex items-center justify-between mb-6">
              <p className="text-[11px] font-black uppercase text-[var(--primary)] tracking-[0.15em]">Results Found: {filteredData.length}</p>
            </div>
            <div className="grid grid-cols-1 gap-2">
              {filteredData.slice(0, 50).map((ex) => (
                <ExerciseRow key={ex.id} exercise={ex} onSelect={toggleExercise} isSelected={selectedExercises.some((s) => s.id === ex.id)} onShowDetails={setDetailExercise} />
              ))}
            </div>
          </div>
        ) : (
          bodyParts.map((part) => {
            const exercisesInPart = exercisesData.filter((ex) => {
              if (part === "back") return BACK_SUB_MUSCLES.some(m => ex.primaryMuscles.includes(m));
            ...
            return (
              <section key={part} className="mb-12">
                <h3 className="font-black uppercase text-sm tracking-[0.2em] flex items-center gap-3 mb-5 text-[var(--text-primary)] border-b border-[var(--border-color)] pb-3">
                  <span className="p-1.5 bg-[var(--primary)]/10 rounded-lg"><Dumbbell size={16} className="text-[var(--primary)]" /></span> 
                  {part} <span className="text-[var(--text-muted)] text-xs font-medium">({exercisesInPart.length})</span>
                </h3>
                <div className="grid grid-cols-1 gap-2">
      ...
      <DetailsModal exercise={detailExercise} onClose={() => setDetailExercise(null)} />
    </div>
  );
}