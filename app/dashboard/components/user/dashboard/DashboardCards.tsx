...
// 1. Use HTMLMotionProps<"div"> to define exactly what a motion div expects
...
export const TodayGoalCard = ({
  ...
  <motion.div
    {...fadeUp(0.05)}
    className={`relative p-5 overflow-hidden rounded-2xl ${planName ? "bg-gradient-to-br from-[var(--primary)] to-[#c45d10]" : "bg-[var(--bg-secondary)] border border-dashed border-[var(--border-color)]"}`}
  >
    <div className="relative z-10 flex items-center justify-between gap-4">
      <div className={planName ? "text-white" : "text-[var(--text-secondary)]"}>
        ...
        {planName ? (
          ...
          <div>
            ...
            <button
              onClick={onSetGoal}
              className="mt-2 text-xs font-black text-[var(--primary)] uppercase"
            >
              Set a Target +
            </button>
          </div>
      ...
      {planName && (
        <div className="shrink-0 font-black text-xl text-white">
          {progress}%
        </div>
      )}
    </div>
...
export const StatCard = ({
  ...
  <motion.div
    {...fadeUp(delay)}
    className="flex flex-col gap-3 p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)]"
  >
    ...
    <div>
      <p className="text-[11px] font-black uppercase tracking-wider mb-1 text-[var(--text-secondary)]">
        {label}
      </p>
