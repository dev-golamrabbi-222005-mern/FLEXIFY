    ...
    <div className="space-y-6 text-[var(--text-primary)]">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">Review applications and manage active coaches</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <StatCard title="Pending Apps" value={analytics?.stats?.pendingCount || 0} icon={Users} color="from-orange-400 to-amber-500" />
        <StatCard title="Active Coaches" value={analytics?.stats?.approvedCount || 0} icon={UserCheck} color="from-green-500 to-emerald-500" />
        <StatCard title="Total Warnings" value={analytics?.stats?.warningCount || 0} icon={AlertTriangle} color="from-red-500 to-pink-500" />
      </div>
                ...
                {filteredCoaches.length > 0 ? filteredCoaches.map((coach) => (
                  <tr key={coach._id.toString()} className="hover:bg-[var(--bg-tertiary)] transition-colors text-sm">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-[var(--text-secondary)]">{coach.email}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 font-semibold text-blue-500">{coach.experienceYears || 0} Years</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${
                        coach.status === 'pending' ? 'bg-amber-500/10 text-amber-500' : 
                        coach.status === 'warning' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'
                      }`}>
                        {coach.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        {activeTab === "pending" ? (
                          <>
                            <button onClick={() => updateStatus.mutate({ id: coach._id.toString(), status: "approved" })} className="p-2 text-green-500 bg-green-500/10 rounded-xl hover:bg-green-500/20 transition" disabled={updateStatus.isPending}><CheckCircle size={18} /></button>
                            <button onClick={() => updateStatus.mutate({ id: coach._id.toString(), status: "rejected" })} className="p-2 text-red-500 bg-red-500/10 rounded-xl hover:bg-red-500/20 transition" disabled={updateStatus.isPending}><XCircle size={18} /></button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => updateStatus.mutate({ id: coach._id.toString(), status: "warning" })} className="p-2 text-amber-500 bg-amber-500/10 rounded-xl hover:bg-amber-500/20 transition" title="Warning" disabled={updateStatus.isPending}><AlertTriangle size={18} /></button>
                            <button onClick={() => updateStatus.mutate({ id: coach._id.toString(), status: "suspended" })} className="p-2 text-red-500 bg-red-500/10 rounded-xl hover:bg-red-500/20 transition" title="Suspend" disabled={updateStatus.isPending}><XCircle size={18} /></button>
                          </>
                        )}
                      </div>
                    </td>
                  ...
                  <tr><td colSpan={5} className="p-16 text-center text-[var(--text-secondary)] font-medium">No records found matching your criteria.</td></tr>
                )}
              </tbody>
            </table>
          </div>
