    ...
    <div className="space-y-8 bg-[var(--bg-primary)] rounded-2xl">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {stats.map(
          (
            stat,
            idx, // <-- Parenthesis starts here
          ) => (
            <div
              key={idx}
              className="bg-[var(--card-bg)] rounded-2xl p-5 border border-[var(--border-color)] hover:shadow-lg transition"
            >
              <div
                className={`w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-r ${stat.color} text-white mb-4 shadow-sm`}
              >
                <stat.icon size={20} />
              </div>
              <h4 className="text-[10px] text-[var(--text-secondary)] font-bold uppercase tracking-widest">
                {stat.title}
              </h4>
      ...
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-2">
        ...
        <div className="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm">
          <h3 className="mb-6 font-bold text-sm uppercase tracking-wider text-[var(--text-secondary)]">
            Revenue Insights
          </h3>
        ...
        <div className="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm">
          ...
          <div className="flex flex-col md:flex-row items-center justify-around h-[250px]">
            <ResponsiveContainer width="60%" height="100%">
              <PieChart>
                <Pie
                  data={dashboard.subscriptionData}
                  innerRadius={60}
                  outerRadius={80}
                  paddingAngle={5}
                  dataKey="value"
        ...
        <div className="bg-[var(--card-bg)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm">
          ...
          <div className="mt-4 flex items-center justify-end gap-2 text-[10px] font-bold uppercase text-[var(--text-secondary)]">
            ...
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className={`w-3 h-3 rounded-sm ${i === 0 ? "bg-gray-200" : i === 1 ? "bg-green-200" : i === 2 ? "bg-green-400" : "bg-green-600"}`}
              />
            ))}
            <span>More</span>
          </div>
