    ...
    <div className="min-h-screen px-6 mx-auto mt-8 md:mt-12 mb-10 max-w-7xl">
      ...
      <div className="flex flex-col gap-3 mb-4 md:flex-row md:items-center">
        <div className="relative flex-grow group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2" size={18} color="var(--text-secondary)" />
          <input
            type="text"
      ...
      {/* ── Filter Panel ── */}
      <div className={`overflow-hidden transition-all duration-500 ${showFilters ? "max-h-[500px] mb-8 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="rounded-3xl p-6" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-color)" }}>
          <div className="flex items-center justify-between mb-5">
            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]">Refine Coaches</p>
            {activeFilterCount > 0 && (
              <button onClick={resetFilters} className="flex items-center gap-1.5 text-[11px] font-black uppercase text-[var(--primary)]">
                <X size={11} /> Clear all
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {/* Specialty Filter */}
            <div>
              <p className="text-[10px] font-black uppercase mb-2.5 text-[var(--text-secondary)]">🎯 Specialty</p>
              <div className="flex flex-wrap gap-1.5">
                {SPECIALTIES.map((s) => (
                  <button
                    key={s}
                    onClick={() => setFilters({ ...filters, specialty: filters.specialty === s ? "" : s })}
            ...
            {/* Experience Filter */}
            <div>
              <p className="text-[10px] font-black uppercase mb-2.5 text-[var(--text-secondary)]">⭐ Experience</p>
            ...
            <div>
              <p className="text-[10px] font-black uppercase mb-2.5 text-[var(--text-secondary)]">👟 Training Type</p>
              <div className="flex flex-col gap-2">
                {TRAINING_TYPES.map((t) => (
                  <button
                    key={t}
                    onClick={() => setFilters({ ...filters, trainingType: filters.trainingType === t ? "" : t })}
      ...
      <div className="relative min-h-[400px]">
        {(isLoading || isFetching) && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-start pt-20 bg-[var(--bg-primary)] bg-opacity-60 backdrop-blur-[2px] transition-all">
            ...
            <p className="text-xs font-black uppercase tracking-widest text-[var(--text-primary)]">Updating Coaches...</p>
          </div>
        )}
          ...
          <>
            {coaches?.length > 0 ? (
              ...
              <div className="text-center py-20">
                <p className="text-[var(--text-secondary)] font-medium">No coaches found matching your criteria.</p>
              </div>
            )}
          </>
        )}
