      ...
      <div className="max-w-7xl mx-auto bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl shadow-lg overflow-hidden font-sans">
        <div className="bg-[var(--primary)] p-8 text-white text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-1 opacity-90">
              Fill up the form to join Flexify team
            </p>
          </div>
        </div>
        ...
        >
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-1">
              <label>Specialties (e.g. Weight Loss, HIIT)</label>
              <input
                {...register("specialties")}
                placeholder="Muscle Gain, Rehab, Yoga"
                className="input-style"
              />
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                {...register("education")}
                placeholder="B.Sc in Sports Science"
                className="input-style"
              />
            </div>
            <div className="flex flex-col gap-1">
          ...
          <section className="space-y-4">
            <h2 className="text-xl font-bold border-b border-[var(--border-color)] pb-2 text-[var(--primary)] uppercase">
              Certifications
            </h2>
            {fields.map((field, index) => (
              <div
                key={field.id}
                className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end p-4 bg-[var(--bg-primary)] rounded-xl border border-[var(--border-color)]"
              >
                <input
                  {...register(`certifications.${index}.title`)}
                  placeholder="Certificate Title (NASM)"
                  className="input-style"
                />
                <input
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-red-500 font-bold px-3 py-2 border border-red-200 rounded-lg hover:bg-red-50 transition-all text-sm"
                  >
                    X
                  </button>
                </div>
            ...
            <button
              type="button"
              onClick={() => append({ title: "", issuedBy: "", year: 2026 })}
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-3">
              <label className="font-bold block underline">
                Training Types
              </label>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                type="number"
                {...register("pricing.perSession", { valueAsNumber: true })}
                placeholder="30"
                className="input-style"
              />
            </div>
          ...
          <button
            ...
            className={`bg-(--primary) rounded-2xl text-white cursor-pointer w-full text-lg md:text-xl lg:text-2xl font-extrabold py-6 uppercase tracking-widest transition-all shadow-xl active:scale-95 relative overflow-hidden ${mutation.isPending ? "opacity-40 cursor-wait" : "hover:brightness-110"}`}
          >
            {/* Dotted Pattern Background */}
            <div
