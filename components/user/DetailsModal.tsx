"use client";
import { X, ChevronRight, ChevronLeft, Target, ShieldCheck } from "lucide-react";
import { useState } from "react";

export const DetailsModal = ({ exercise, onClose }: any) => {
          ...
          <div className="min-w-0">
            ...
            <div className="flex gap-2 items-center">
              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-lg border ${
                exercise.level === 'beginner' ? 'text-emerald-500 border-emerald-500/20 bg-emerald-500/5' : 
                exercise.level === 'intermediate' ? 'text-amber-500 border-amber-500/20 bg-amber-500/5' : 'text-rose-500 border-rose-500/20 bg-rose-500/5'
              }`}>
                {exercise.level}
              </span>
              ...
              {exercise.images.length > 1 && (
                <>
                  <div className="absolute inset-0 flex items-center justify-between px-4 pointer-events-none">
                    <button 
                      onClick={() => setCurrentImg(prev => prev === 0 ? exercise.images.length - 1 : prev - 1)}
                    ...
                    <button 
                      onClick={() => setCurrentImg(prev => prev === exercise.images.length - 1 ? 0 : prev + 1)}
                      className="pointer-events-auto bg-white/90 backdrop-blur-md p-3 rounded-2xl shadow-xl text-black hover:bg-[var(--primary)] hover:text-white transition-all active:scale-90"
                    >
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 bg-black/20 backdrop-blur-md px-3 py-1.5 rounded-full">
                    {exercise.images.map((_: any, i: number) => (
                      <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${currentImg === i ? 'w-6 bg-white' : 'w-1.5 bg-white/40'}`} />
                    ))}
                  </div>
                </>
              )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-[var(--bg-tertiary)] p-4 rounded-3xl border border-[var(--border-color)] flex items-center gap-3">
               ...
               <div>
                  ...
                  <p className="text-[11px] font-bold uppercase text-[var(--text-primary)] truncate">{exercise.equipment || "Bodyweight"}</p>
               </div>
            </div>
          </div>
