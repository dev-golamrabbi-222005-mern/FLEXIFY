  ...
  return (
    <section className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-[var(--shadow-sm)] transition-all">
      ...
      <div className="p-4 border-b border-[var(--border-color)] flex justify-between items-center bg-[var(--bg-tertiary)]/30">
        <h2 className="font-black uppercase text-sm tracking-tight text-[var(--text-primary)]">
          <span className="text-[var(--primary)] mr-2">#{exIdx + 1}</span>
          {exercise.name}
        </h2>
      ...
      <div className="p-4 space-y-3">
        {exercise.sets.map((set, sIdx) => (
          <div
            key={set.id}
            className={`grid grid-cols-12 gap-3 items-center p-3 rounded-xl border transition-all duration-300 ${
              set.isCompleted
          ...
          >
            ...
            <div className="col-span-1 text-[10px] font-black text-[var(--text-muted)] text-center">
              {sIdx + 1}
            </div>
            <div className="col-span-2 text-[10px] font-medium  text-[var(--text-muted)] flex items-center gap-1 truncate">
              ...
              {exercise.trackingType !== "TIME" ? (
                <>
                  <input
                    ...
                    value={set.weight}
                    onChange={(e) =>
                      updateInput(sIdx, "weight", e.target.value)
                    }
            ...
            <div className="col-span-2 flex justify-end">
              <button
                onClick={() => {
                  const capturedTime =
                    activeTimerSet === set.id ? `${time}s` : undefined;
                  onToggleComplete(exIdx, sIdx, capturedTime);
                }}
                className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
                  set.isCompleted 
                    ? "bg-[var(--primary)] text-white scale-95 shadow-lg"
